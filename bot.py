...
• `/update_words` — обновить словарь нецензурных слов из Викисловаря
• `/force_update` — принудительно обновить словарь с удалением кеша
• `/add_word [слово]` — добавить новое слово в словарь
• `/debug` — показать информацию о текущем словаре
...
async def debug_info(message: types.Message):
    ...
    debug_text = f"📊 *Информация о словаре:*\n\n" \
...
async def add_bad_word(message: types.Message):
    """
    Добавление слова в словарь нецензурной лексики (только для администратора)
    """
    if not is_admin(message.from_user.id):
        await message.reply("⚠️ У вас нет прав администратора для выполнения этой команды.")
        return
